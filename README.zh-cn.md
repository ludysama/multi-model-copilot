<h1 align="center">多模型 Copilot（DeepSeek V4 + GLM-5.2）</h1>

<p align="center">
  <!-- marketplace-readme:remove-start -->
  <a href="https://marketplace.visualstudio.com/items?itemName=ludysama.multi-model-copilot"><img src="https://img.shields.io/badge/VS%20Code%20Marketplace-Install-007ACC?logo=visualstudiocode&logoColor=white&style=for-the-badge" alt="从 VS Code Marketplace 安装"></a>
  <a href="https://open-vsx.org/extension/ludysama/multi-model-copilot"><img src="https://img.shields.io/badge/Open%20VSX-Install-6A4FB6?style=for-the-badge" alt="从 Open VSX 安装"></a>
  <br/>
  <!-- marketplace-readme:remove-end -->
  <img src="https://img.shields.io/github/v/release/ludysama/multi-model-copilot?style=for-the-badge&label=Version" alt="版本" />
  <img src="https://vsmarketplacebadges.dev/installs-short/ludysama.multi-model-copilot.svg?style=for-the-badge" alt="安装量" />
</p>
...
> 
> **原作者：[Vizards](https://github.com/Vizards)** — DeepSeek V4 Copilot Chat 集成的核心工作归功于原作者。
> 本 Fork 在上游基础上扩展了**多模型支持**（GLM-5.2 + 按模型独立路由 API Key），后续计划加入更多模型提供方。
> 基于 MIT 协议，与上游一致。
> 
> 📦 [本 Fork 仓库](https://github.com/ludysama/multi-model-copilot) · 🔗 [原始上游](https://github.com/Vizards/deepseek-v4-for-copilot)

---

**在 Copilot Chat 模型选择器中直接使用 DeepSeek V4 或智谱 GLM-5.2——无需离开你熟悉的 Copilot 工作流。**
...
喜欢 DeepSeek / 智谱的性价比，但不想放弃 GitHub Copilot 的 Agent 模式、工具调用和成熟的交互体验？本扩展将 **DeepSeek V4 Pro 和 Flash** 以及 **智谱 GLM-5.2** 直接接入 Copilot Chat 模型选择器，支持**视觉识别**、**思考模式**，使用你自己的、按提供方独立保存的 API Key。多个提供方可以并存。
...
### DeepSeek V4 Pro & Flash + 智谱 GLM-5.2 出现在模型选择器中
三个模型与 GPT-4o、Claude 等并列在 Copilot Chat 的模型选择器中。均支持 1M Token 上下文。可在对话中途切换模型，不丢失聊天历史。
...
- VS Code 1.116 及以上版本。本扩展依赖非公开的 Copilot Chat API，较新的 VS Code 版本可能存在兼容性问题——如遇到请[提交 Issue](https://github.com/ludysama/multi-model-copilot/issues)。
...
- （可选）智谱 GLM API Key，从 [bigmodel.cn](https://bigmodel.cn/usercenter/proj-mgmt/apikeys) 获取，用于启用 GLM-5.2

### 安装方式

根据你所使用的编辑器选择对应的注册表安装：

1. **Microsoft VS Code** — 从 [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=ludysama.multi-model-copilot) 安装。
2. **使用 Open VSX 的编辑器** — 从 [Open VSX](https://open-vsx.org/extension/ludysama/multi-model-copilot) 安装。

### 使用步骤

1. 通过命令面板（`Cmd+Shift+P`）运行 **多模型 Copilot: 设置 API Key**
2. 会弹出 QuickPick 列出所有支持的提供方——选一个并粘贴对应的 Key。对每个想使用的提供方重复此操作（官方 DeepSeek Key 通常以 `sk-` 开头）
3. 打开 Copilot Chat，点击模型选择器，选择 **DeepSeek V4 Pro**、**DeepSeek V4 Flash** 或 **GLM-5.2（智谱）**
4. 搞定——开始聊天
...
| **GLM-5.2（智谱）** | 1M 上下文、OpenAI 兼容端点、价格有竞争力 |

DeepSeek V4 支持可选的思考模式、工具调用和 1M Token 上下文。GLM-5.2 支持工具调用和 1M Token 上下文。
...
| `deepseek-copilot.debugMode` | `minimal` | 诊断模式：`minimal` 仅上报 token 用量，`metadata` 输出隐私安全日志，`verbose` 将完整请求 dump 和 pipeline snapshot 写入扩展 global storage。完整 dump 可能包含敏感提示词文本、工具定义、文件片段和图片描述。使用 `多模型 Copilot: 打开请求 Dump 目录` 打开 dump 位置 |
| `deepseek-copilot.visionModel` | *(自动)* | 用作图片代理的 VS Code 视觉模型。请通过 `多模型 Copilot: 配置视觉代理` 设置；新版保存为 `vendor/id`，旧版裸模型 ID 仍兼容读取 |
